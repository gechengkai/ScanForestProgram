...
from PyQt5.QtWidgets import QApplication, QWidget, QTabWidget, QPushButton, QLabel,QComboBox, QLineEdit, QGridLayout, QDesktopWidget, QHBoxLayout, QProgressBar, QStackedWidget
...
class TianwaicunWindow(QWidget):
    def __init__(self):
        super().__init__() # 初始化
        self.initUI()  # 初始化界面
        self.twc_positionList = []
        self.twc_savePath = "detectDir/tianwaicun/capImg/"  
        self.mkdir(self.twc_savePath)
    ...
    def startBtnSlot(self):
        ...
        # 开始截图
        self.twc_captrueImgThread = TianwaicunCaptureThread(self.twc_positionList)
        #self.twc_captrueImgThread.startCaptureSig.connect(self.twc_prgBar.setMaximum)
        # 展示抓取到的图片
        self.twc_captrueImgThread.getImageSig.connect(self.displyImage)

        self.twc_captrueImgThread.start()
    ...
    def displyImage(self,imgPathstr):
        print("显示图片")
        png = QtGui.QPixmap("detectDir/tianwaicun/capImg/twc.jpg")
        png.scaled(650,365)
