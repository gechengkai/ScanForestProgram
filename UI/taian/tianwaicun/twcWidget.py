...
class TianwaicunCaptureThread(QThread):    
    # 开始抓图
    ...
    def __init__(self,positionList):
        ...
        #全局变量
        self.savePath = "detectDir/tianwaicun/capImg/" # where are these images that were cptured to store
        self.positionList = positionList
        self.num = 0
        self.save_path = "./detectDir/tianwaicun/detect/" # 
        self.curImgSavePath = "detectDir/tianwaicun/capImg/"
    ...
    def startWork(self):
        ...
        if capImgRes != None:
            ...
            photo.save("detectDir/tianwaicun/capImg/twc.jpg")
            self.num += 1
            #发送展示抓取图像信号
            self.getImageSig.emit("detectDir/tianwaicun/capImg/twc.jpg")
        else:
            print("天外村图片抓取失败")
    ...
    def capImgRequest(self):
        ...
        #组成json
        postData = dict(cameraIndexCode="c0c45e5aa90e46daa50a4642c10fa624")
        setPostionUrl = "http://112.245.48.4:8089/hey/captures"
        #print(setPostionUrl)
        setPostionResponds = requests.post(setPostionUrl, json=postData)
        print("setPostionResponds.text:%s"%setPostionResponds.text)
