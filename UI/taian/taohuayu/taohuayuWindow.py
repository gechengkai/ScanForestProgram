...
class TaohuayuWindow(QWidget):
    def __init__(self):
        super().__init__() # 初始化
        self.initUI()  # 初始化界面
        self.thy_positionList = []
        self.thy_savePath = "detectDir/taohuayu/capImg/"
        self.mkdir(self.thy_savePath)
    ...
    def startBtnSlot(self):
        ...
        # 开始截图
        self.scan_captrueImgThread = TaohuayuCaptureThread(self.thy_positionList)
        #self.twc_captrueImgThread.startCaptureSig.connect(self.twc_prgBar.setMaximum)
        # 展示抓取到的图片
        self.scan_captrueImgThread.getImageSig.connect(self.displyImage)

        self.scan_captrueImgThread.start()
    
    #显示抓取到的图片
    def displyImage(self,imgPathstr):
        print("显示图片")
        png = QtGui.QPixmap("detectDir/taohuayu/capImg/thy.jpg")
        png.scaled(650,365)
