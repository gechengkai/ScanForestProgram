...
# -*- encoding: utf-8 -*-
'''
@Description:    :实现了利用威海摄像头功图像获取、图像识别、图像上传服务器三个类
@Date            :2021/09/02 11:38:21
@Author          :gechengkai
@version         :v1.0
'''
import os, sys, requests, json, time, base64, os, socket, urllib3.request, glob, argparse, threading, logging, logging.handlers
...
class WeiHaiCaptureThread(QThread):    
    # 开始抓图
    ...
    def __init__(self,positionList):
        super().__init__()
        # logger使用方法1使用，配置handler( 方法2是使用logging.basicConfig()，此处不用 )
        # 创建一个logger 
        self.logger = logging.getLogger("shuangdaowan_capture") # 初始化logger
        self.logger.setLevel(logging.INFO) # Log 等级总开关
        # 创建一个FileHandler，用于写入日志文件
        handler_file = logging.FileHandler(filename="shuangdaowan.log", mode='a', encoding='utf-8')
        handler_file.setLevel(logging.INFO) #输出到 file 的 Log 等级
        # 创建一个StreamHandler，用来把日志写到cmd上
        ...
        #全局变量
        self.savePath = "detectDir\\weihai\\capImg\\" # where are these images that were cptured to store
        self.positionList = positionList # 抓取图片摄像机位置参数列表
        self.num = 0
        self.save_path = ".\\detectDir\\weihai\\detect\\" # 
        self.curImgSavePath = "detectDir\\weihai\\capImg\\"
    ...
    def startWork(self,flag):
        if flag == "init":
            ...
            for i in range(1695, len(self.positionList)):
                self.num  = i
                self.logger.info("\n\n————————————————————————————————  %d  ————————————————————————————————"%int(self.num))
                self.logger.info("work:威海-第%d张图片" %(i))
                self.logger.info("work:威海-共%d张图片" %(len(self.positionList)))
                # 设定位置
            ...
            for i in range(self.num, len(self.positionList)):
                self.logger.info("\n\n————————————————————————————————  %d  ————————————————————————————————"%int(self.num))
                self.logger.info("work:retry-威海-第%d张图片"%i)
                self.logger.info("work:retry-威海-共%d张图片"%(len(self.positionList)))
                self.num = i
                # 设定位置
    ...
    def setPositonRequest(self, positionStr):
        ...
        if token != None:
            ...
            #组成json
            postData = dict(camera_id="37090200001311001014", pan = int(panStr), tilt=int(tiltStr), zoom=int(zoomStr))
    ...
    def cptureRequests(self):
        ...
        if token != None:
            ...
            backStr = "&camera_id=37090200001311001014&stream_type=0&is_local=1&data_mode=1&pic_num=1"
    ...
    def getCameraPositon(self):
        ...
        if token != None:
            ...
            backStr = "&camera_id=37090200001311001014"
            cptureUrl = frontStr + token + backStr
            try:
                cptureResponse = requests.get(cptureUrl,timeout=(3,7))
            except:
...
class WeiHaiPredictThread(QThread):
    # 更新进度条
    setPrgBarValSig = QtCore.pyqtSignal(int)
    # 开始检测
    ...
    def __init__(self):
        ...
        # 创建一个logger 
        self.logger = logging.getLogger("shuangdaowan_predict") # 初始化logger
        self.logger.setLevel(logging.INFO) # Log 等级总开关
        ...
        self.sourcePaht = "detectDir\\weihai\\capImg" # 检测文件夹
        self.inferRestSavePath = "detectDir\\weihai\\detect\\" # 检测结果保存总文件夹
        self.uploadImgPath = "" # 传递给上传类的参数，指向检测结果保存目录
        self.disPlayImgPaht = "" # 保存检测过程中要展示在界面上的图片的路径
        

    def run(self):
        self.logger.info("启动病树检测线程------------->")
        self.startInference()

    @torch.no_grad()
    def inference(self, weights='best.pt',  # model.pt path(s)
            source='detectDir\\weihai\\capImg',  # file/dir/URL/glob, 0 for webcam
...
class WeiHaiUploadThread(QThread):
    # 更新进度条
    setPrgBarValSig = QtCore.pyqtSignal(int)
    # 开始上传
    ...
    def __init__(self,resultSavePath):        
        ...
        # logger使用方法1使用，配置handler( 方法2，使用logging.basicConfig() )
        # 创建一个logger 
        self.logger = logging.getLogger("shuangdaowan_upload") # 初始化logger
        self.logger.setLevel(logging.INFO) # Log 等级总开关
        # 创建一个handler，用于写入日志文件
        handler_file = logging.FileHandler(filename="shuangdaowan.log", mode='a', encoding='utf-8')
        handler_file.setLevel(logging.INFO) #输出到 file 的 Log 等级
    ...
    def sendMsg(self, img_name):
        ...
        postData = dict(treeNum=str(int(round(t * 1))), workContent = "普查", img1=img_name, equipment="37090200001311001014", source="摄像头")
