...
# -*- encoding: utf-8 -*-
'''
@Description:    :实现了利用烟台摄像头功图像获取、图像识别、图像上传服务器三个类
...
class YantaiCaptureThread(QThread):    
    # 开始抓图
    ...
    def __init__(self,positionList):
        ...
        # 创建一个logger 
        self.logger = logging.getLogger("muping_capture") # 初始化logger
        self.logger.setLevel(logging.INFO) # Log 等级总开关
        # 创建一个handler，用于写入日志文件
        handler_file = logging.FileHandler(filename="muping.log", mode='a', encoding='utf-8')
        ...
        #全局变量
        self.savePath = "detectDir\\yantai\\capImg\\" # where are these images that were cptured to store
        self.positionList = positionList # 抓取图片摄像机位置参数列表
        self.num = 0
        self.save_path = ".\\detectDir\\yantai\\detect\\" # 
        self.curImgSavePath = "detectDir\\yantai\\capImg\\"
    ...
    def startWork(self,flag):
        if flag == "init":
            ...
            for i in range(4107,len(self.positionList)):
                self.num  = i
                self.logger.info("\n\n————————————————————————————————  %d  ————————————————————————————————"%int(self.num))
                self.logger.info("work:烟台-第%d张图片" %(i))
                self.logger.info("work:烟台-共%d张图片" %(len(self.positionList)))
                # 设定位置
            ...
            for i in range(self.num, len(self.positionList)):
                self.logger.info("\n\n————————————————————————————————  %d  ————————————————————————————————"%int(self.num))
                self.logger.info("work:retry-烟台-第%d张图片"%i)
                self.logger.info("work:retry-烟台-共%d张图片"%(len(self.positionList)))
                self.num = i
                # 设定位置
    ...
    def setPositonRequest(self, positionStr):
        ...
        if token != None:
            ...
            #组成json
            postData = dict(camera_id="37090200001311000986", pan = int(panStr), tilt=int(tiltStr), zoom=int(zoomStr))
    ...
    def cptureRequests(self):
        ...
        if token != None:
            ...
            backStr = "&camera_id=37090200001311000986&stream_type=0&is_local=1&data_mode=1&pic_num=1"
    ...
    def getCameraPositon(self):
        ...
        if token != None:
            ...
            backStr = "&camera_id=37090200001311000986"
            cptureUrl = frontStr + token + backStr
            try:
                cptureResponse = requests.get(cptureUrl,timeout=(3,7))
            except:
...
class YantaiPredictThread(QThread):
    # 更新进度条
    setPrgBarValSig = QtCore.pyqtSignal(int)
    # 开始检测
    ...
    def __init__(self):
        ...
        # logger使用方法1使用，配置handler( 方法2，使用logging.basicConfig() )
        # 创建一个logger 
        self.logger = logging.getLogger("muping_predict") # 初始化logger
        self.logger.setLevel(logging.INFO) # Log 等级总开关
        # 创建一个handler，用于写入日志文件
        handler_file = logging.FileHandler(filename="muping.log", mode='a', encoding='utf-8')
        handler_file.setLevel(logging.INFO) #输出到 file 的 Log 等级
        ...
        self.sourcePaht = "detectDir\\yantai\\capImg" # 检测文件夹
        self.inferRestSavePath = "detectDir\\yantai\\detect\\" # 检测结果保存总文件夹
    ...
    def inference(self, weights='best.pt',  # model.pt path(s)
            source='detectDir\\yantai\\capImg',  # file/dir/URL/glob, 0 for webcam
...
class YantaiUploadThread(QThread):
    # 更新进度条
    setPrgBarValSig = QtCore.pyqtSignal(int)
    # 开始上传
    ...
    def __init__(self,resultSavePath):        
        ...
        # logger使用方法1使用，配置handler( 方法2，使用logging.basicConfig() )
        # 创建一个logger 
        self.logger = logging.getLogger("muping_upload") # 初始化logger
        self.logger.setLevel(logging.INFO) # Log 等级总开关
        # 创建一个handler，用于写入日志文件
        handler_file = logging.FileHandler(filename="muping.log", mode='a', encoding='utf-8')
        handler_file.setLevel(logging.INFO) #输出到 file 的 Log 等级
    ...
    def sendMsg(self, img_name):
        ...
        postData = dict(treeNum=str(int(round(t * 1))), workContent = "普查", img1=img_name, equipment="37090200001311000986", source="摄像头")
